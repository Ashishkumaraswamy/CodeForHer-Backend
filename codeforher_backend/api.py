...
from codeforher_backend.services.ola_service import OLAService
...
@app.post(f"{BASE_URL}/ola/get-route")
def get_route(request: RouteRequest, token: str = Depends(oauth2_scheme)):
    verify_token(service_config.jwt_config, token)
    LOG.info("User authenticated with the token successfully")

    ola_service = OLAService(service_config)
    response = ola_service.get_route(request)
    return response

@app.post(f"{BASE_URL}/ola/get-time-distance")
...
@app.post(f"{BASE_URL}/ola/nearby-safe-spots")
